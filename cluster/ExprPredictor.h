...
double getRMSEdc( string& file ) ;
  ...
  double PvalueSumf( double alpha ) ;  // constructs score cdf using selection fequencies (as opposed to backgroud)
	...
	double PvalueSumdc( double alpha , string& name ); //name, holds the file that will be filled by pvdc.txt (the energy verse cdf)
	double PvalueSumdu( double alpha , string& name );
	double PvalueSumcb( double alpha, string& name  );
...
private:
   ...
   // Matrix pwm2;
	...
	double mutualinformatio( const vector< Motif >& _motifs ,double alphadc );
...
int annotydorsalZarend2u2( vector< Sequence > seq, vector< vector< vector< Site > > >& sitespp , Matrix f, Matrix e, ExprFunc& p, int column, vector< string >  names, vector< vector< Site > >& seqSitesbot, vector< vector< Site > >& seqSitesm1);
...
double compAvgCorr4( ExprPar& par  , int thres) ;
