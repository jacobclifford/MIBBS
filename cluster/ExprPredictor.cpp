...
void fastaflip( string& filein , string& fileout)
{
string file = filein;
...
int l = enepwm.nRows();
    ...
    for ( int i = 0; i < enepwm.nRows(); i++ ) {
        for ( int j = 0; j < enepwm.nCols(); j++ ) {
            Expected_info = Expected_info - enepwm.getElement( i, j ) * log2( enepwm.getElement(i,j) );	
        }	
    }


return (l*2 - Expected_info );  
}					
...
int SeqAnnotator::annotI( const Sequence& seq, SiteVec& sites ) const
{
    sites.clear();
    ...
    for ( int i = 0; i < seq.size(); i++ ) {
        ...
        for ( int k = 0; k < motifs.size(); k++ ) {
            ...
            energy = motifs[ k ].information( elem );
                ...
                sites.push_back( Site( i, 1, energy ) );
            ...
            energy = motifs[ k ].information( rcElem );
                ...
                sites.push_back( Site( i, 0, energy ) );
...
Site SeqAnnotator::siteMaxI( vector< Site >& sitess)
{
 Site tempsite;
 double minI=0;
 for ( int i = 0; i < sitess.size(); i++ ) {
	...
	if ( minI < sitess[i].information ){  
		minI = sitess[i].information;
		tempsite = sitess[i];
 	}
  }
 return tempsite;	
...
void alignNEW(string& file,int nrandStarts, int nUpdates , int wid, int l , int inup, int knmo   ){
int D =0;
string seqFile, dcFile, twFile ;
seqFile=file;
...
double pwm_bestt = 1;
double pwm_model2t =1.1;
double pwm_modelt= 4;
double pwm_shift_bestt = 1;
double pwm_shift0t = 1;
double pwm_shiftt = 1;
int pin;
vector< int > ipn(nSeqs,0);	
vector< int > isn(nSeqs,0);
vector< int > stran(nSeqs,0);
int initp =0;  
vector< int > pos(nSeqs,initp);  
vector<double > sensitivSites(nSeqs,0);

vector< int > strant(nSeqs,0);
vector< int > post(nSeqs,initp);  

vector< int > posbest(nSeqs,initp);
ofstream scor("scores.txt");
ofstream shik("text.txt");
for(int j=0; j< 10 ; j++ ){
shik << j <<" " ;
}
shik << endl;
...
vector< Sequence > seqst;
int nSeqst = nSeqs;
        vector< string > seqNamest;
		...
		for(int i = 0; i < nSeqs; i++){
		   while( true ){
			...
			flagif = true;
			     ...
			     if( D3t + pos[i] > seqs[i].size() ){ flagcoor = false;  flagif =false;
				pos[i] = 5; 
			     }
			...
			flagif =true;
			     ...
			     if( pos[i] + D5b > seqs[i].size() ){  flagcoor = false;  flagif =false;
				pos[i] =5; 
			     }
			}
		    	if( flagif == true) break;  
		     } 
...
for ( int k = 0; k < nUpdates; k++ ) {
	...
	dcFile = "corefile.txt";
	twFile = "corefilet.txt";
	vector< Motif > motifs2;
	vector< Motif > motifs2t;
 ...
 for ( int j = 0; j < seqs.size(); j++ ) {       
	SiteVec seqSites;
	 ann.annotI( seqs[ j ], seqSites );
	 Site m = ann.siteMaxI( seqSites );
	int p = m.start;
	int st= m.strand;
	stran[j] = st;
	pos[j] = p ;	
	...
	pwm_model2 = dcmm2.getInformation();  
		...
		if ( pwm_model2  > pwm_best  ) {
			...
			for( int jj = 0; jj < nSeqs ; jj++) {
				...
				ipf << start << '\t' ;
				ipn.push_back( start );
				isn.push_back( stran[jj] );
					if(pos[jj] + l > seqs[jj].size() ) { start = 1;  } 
			optcore << ">" <<  seqNames[jj] << "_" << ipn[jj] << "_"<<isn[jj]<< endl;
			...
			if(stran[jj]){
				...
				for( int ii = 0; ii < l ; ii++) {  
				 int p = start ;
				optcore << ALPHABET[complement(symbolToInt(ALPHABET[ seqs[jj][p+ l -1 - ii ] ]))];;
				}
	...
	for(int shif = 0; shif < 4 ; shif++ ){
		...
		ostringstream numberm;
			...
			for( int jj = 0; jj < nSeqs ; jj++) {
				...
				if(pos[jj] + l + shif + D > seqs[jj].size() ) { miss=miss +1; shift = 0; }  
				
			if(stran[jj]){
				
				core3 << ">" <<  seqNames[jj] << "_" << ipn[jj] << "_"<<isn[jj]<< endl;
				
				for( int ii = 0; ii < l ; ii++) {  
				 int p = start + ii + shift;
				...
				if(  start -shif <= 0 || startt - shif <=0 ){miss=miss +1;   shift = 0;}  
			...
			if( pwm_shift_best  > pwm_shift0  ){     
				for( int jj = 0; jj < nSeqs ; jj++) {		
					if(stran[jj]){
						...
						if(pos[jj] + l + shif + D > seqs[jj].size() ) {   continue; }
						pos[jj] = pos[jj] + shif;
						
					} 
					else{ 
						 if(  pos[jj] -shif <= 0  ){  continue;}
						 pos[jj] =pos[jj] - shif; 
					}
				}
			} 
	...
	for(int shif = 0; shif < 4 ; shif++ ){
		...
		ostringstream numberm;
			...
			if(stran[jj]){
				if(  start -shif <= 0){miss=miss +1;   shift = 0;}  
		...
		for(int shif = 0; shif < 3 ; shif++ ){

		ostringstream numberm;
			numberm << shif;
		ostringstream numbermi;
			numbermi << inup;
		ostringstream numbermk;
			numbermk << knmo;
			string dcFile2= "coredc"+ numbermi.str() + "OPT" + numberm.str()+ ".txt";
				ofstream coreshif( dcFile2.c_str() );
			...
			if(isn[jj]){
				...
				coreshif << ">" <<  seqNames[jj] << "_" << ipn[jj] << "_"<<isn[jj] <<endl;
				int ll = l + 2*shift;
				for( int ii = 0; ii < ll ; ii++) {  
				 int p = start + ii - shift;
				if( p  >= seqs[jj].size()  || p < 0  ) { coreshif << "n" ; continue; }
				coreshif << ALPHABET[ seqs[jj][p] ];
				}
				...
				for( int ii = 0; ii < ll ; ii++) {  
				 int p = start - shift ;
				 if(p+ll-1 >= seqs[jj].size()  || p < 0  ) { coreshif << "n" ; continue; }
				coreshif << ALPHABET[complement(symbolToInt(ALPHABET[ seqs[jj][p+ ll -1 - ii ] ]))];;
				}
...
void alignNEW2(string& file,int nrandStarts, int nUpdates , int wid, int l , int inup, int knmo   ){
int D =0;
string seqFile, dcFile, twFile ;
seqFile=file;
 ...
 for ( int j = 0; j < seqs.size(); j++ ) {       
	...
	pwm_model2 = dcmm2.getInformation();  
		...
		if ( pwm_model2  > pwm_best  ) {
			...
			for( int jj = 0; jj < nSeqs ; jj++) {
				...
				isn.push_back( stran[jj] );
					if(pos[jj] + l > seqs[jj].size() ) { start = 1;  } 
			optcore << ">" <<  seqNames[jj] <<"_" << ipn[jj]<<"a" << "_"<<isn[jj]<< endl;
	...
	for(int shif = 0; shif < 0 ; shif++ ){
		...
		ostringstream numberm;
			...
			if(stran[jj]){
				if(  start -shif <= 0){miss=miss +1;   shift = 0;}  
				core3 << ">" <<  seqNames[jj] <<"_" << ipn[jj] <<"b"<< "_"<<isn[jj]<< endl;
		...
		ostringstream numbermk;
			numbermk << knmo;
			string dcFile2= "coredu"+ numbermi.str() + "OPT" + numberm.str()+ ".txt";
				ofstream coreshif( dcFile2.c_str() );
...
void alignNEW3(string& file,int nrandStarts, int nUpdates , int wid, int l , int inup, int knmo   ){
int D =0;
string seqFile, dcFile, twFile ;
seqFile=file;
...
for ( int k = 0; k < nUpdates; k++ ) {
 ...
 for ( int j = 0; j < seqs.size(); j++ ) {       
	...
	for(int shif = 0; shif < 2 ; shif++ ){
		...
		ostringstream numbermk;
			numbermk << knmo;
			string dcFile2= "core"+ numbermi.str() + "OPT" + numberm.str()+ ".txt";
				ofstream coreshif( dcFile2.c_str() );
...
double Motif::PvalueSumdc( double alpha, string& name  ) 
{
int l = pwm.nRows();
vector< double > freqs;
vector< double > eners;
...
double Motif::PvalueSumdu( double alpha, string& name  ) 
{
int l = pwm.nRows();
vector< double > freqs;
vector< double > eners;
...
double Motif::PvalueSumcb( double alpha , string& name ) 
{
int l = pwm.nRows();
vector< double > freqs;
vector< double > eners;
...
int SeqAnnotator::annotydorsalZarend2u2( vector< Sequence > seq, vector< vector< vector< Site > > >& sitespp , Matrix f, Matrix e, ExprFunc& p, int column, vector< string >  names, vector< vector< Site > >& seqSitesbot, vector< vector< Site > >& seqSitesm1)
{
Site t;
vector< double > ff(3);  
 vector< vector< vector< Site > > > sitespt(seq.size(), vector< vector< Site > >(4));
  ...
  for ( int k = 0; k < motifs.size(); k++ ) {  
	...
	for ( int e = 0; e < seq.size(); e++ ) {  
	  for ( int k = 0; k < motifs.size(); k++ ) {  
		...
		fff << "sitespt[e][k]" << sitespt[e][k] << endl;
	  } 
	} 
	 fff << "motifs " << endl << motifs << endl;
	fff<< "motifs 0 LLRMAT " << endl << motifs[0].getLLRMatpos() << endl;
	fff<< "motifs 1 LLRMAT " << endl << motifs[1].getLLRMatpos() << endl;
	fff << sitespt << endl;
	 fff.close();
	for ( int e = 0; e < seq.size(); e++ ) {  
	  for ( int k = 0; k < motifs.size(); k++ ) {  
...
int numsi=0;
for( int e = 0; e < seq.size(); e++ ) {
	sitespp[e][0] = seqSitesbot[e];    
	numsi = sitespp[e][0].size() + numsi;
	sitespp[e][1] = seqSitesm1[e];      
}
ecounts.close();
...
for ( int e = 0; e < _d.size(); e++ ) {  
		...
		for(int j =0; j < _d[e][0].size(); j++ ) {
			...
			for( int i = 0; i < tsites.size() ; i++ ) {	
				...
				du << ">" << ExprPredictor::seqNmesa[e] << "_" << tsites[i].start <<endl ;
					int shift = motifs[ tsites[i].factorIdx ].length();
				for( int p =0 ; p < shift; p++ ){
					jj = tsites[i].start ;
					if(tsites[i].strand){
						du <<  ALPHABET[ ExprPredictor::seqsya[e][ jj+p ]  ] ;
					}
					else{ jj=  tsites[i].start; du << ALPHABET[complement(symbolToInt(ALPHABET[ ExprPredictor::seqsya[e][jj+ shift -1 - p ] ]))];} 
				} 
				du << endl;
				break;  
				} 
		...
		for(int t=0; t< _d[e][2].size();t++ ) {
			...
			if( dist <B[2] && dist >= B[1] || (dist ==0 && B[3] ==300) ){ 
				...
				for( int i = 0; i < tsites.size() ; i++ ) {	
				   if( tsites[i].start == a.start ) { 
					...
					dc << ">" << ExprPredictor::seqNmesa[e] << "_" << tsites[i].start << "_" << motifs[ tsites[i].factorIdx ].length() <<"_" <<b.start <<"_"<< dist   <<endl ;
					int shift = motifs[ tsites[i].factorIdx ].length();
					for( int p =0 ; p < shift; p++ ){
						jj = tsites[i].start;
						if(tsites[i].strand){
						...
						else{ jj=  tsites[i].start ;  dc << ALPHABET[complement(symbolToInt(ALPHABET[ ExprPredictor::seqsya[e][jj+ shift -1 - p ] ]))];} 
					} 
					dc << endl;
					flagd=1;
					flagt=1;
					...
					vector< Site > tsites;
						...
						Site a =_d[e][1][j];
							for( int i = 0; i < tsites.size() ; i++ ) {	
								...
								du << ">" << ExprPredictor::seqNmesa[e] << "_" << tsites[i].start<< motifs[ tsites[i].factorIdx ].length() <<endl ;
								int shift = motifs[ tsites[i].factorIdx ].length();
								for( int p =0 ; p < shift; p++ ){
									jj = tsites[i].start  ;
									if(tsites[i].strand){
			...
			if( flagd==1){ break;}
				...
				vector< Site > tsites;
					...
					for( int i = 0; i < tsites.size() ; i++ ) {	
						...
						du << ">" << ExprPredictor::seqNmesa[e] << "_" << tsites[i].start<<"_"<< b.start <<"_"<< dist  <<endl ;
						int shift = motifs[ tsites[i].factorIdx ].length();
						for( int p =0 ; p < shift; p++ ){
							jj = tsites[i].start ;
							if(tsites[i].strand){
...
for(int m = 0; m < ExprPredictor::seqsya.size(); m++ ) {
	...
	for( int i = 0; i < tsites.size() ; i++ ) {			
				...
				if(  tsites[i].start-30  <= 0 ){continue;}

				if(  tsites[i].start+motifs[ tsites[i].factorIdx ].length() + 30 > ExprPredictor::seqsya[m].size() ){continue;}
					 int j = 0;
					dc << ">" << ExprPredictor::seqNmesa[m] << "_" << tsites[i].start << "_" << tsites[i].strand<<endl ;
					for( int p =0 ; p < motifs[ tsites[i].factorIdx ].length() +20; p++ ){
						j = tsites[i].start-10 ;
						if(tsites[i].strand){
						
						dc <<  ALPHABET[ ExprPredictor::seqsya[m][ j+p ]  ] ;
						}
					else{ j=  tsites[i].start+ 10 ; dc << ALPHABET[complement(symbolToInt(ALPHABET[ ExprPredictor::seqsya[m][j+ motifs[ tsites[i].factorIdx ].length() -1 - p ] ]))];} 				
						} 
				...
				if( tsites[i].factorIdx == 1 ) { 
					...
					du << ">" << ExprPredictor::seqNmesa[m] << "_" << tsites[i].start << "_" << tsites[i].strand<<endl ;
					for( int p = 0; p < motifs[ tsites[i].factorIdx ].length() ; p++ ){
...
double ExprPredictor::compAvgCorr4(  ExprPar& par, int thres ) 
{
...
Matrix e = exprData;
	   ...
	   anny.annotydorsalZarend2u2( seqsya, d, f, e, *func , thres, ExprPredictor::seqNmesa, seqSitesbot, seqSitesm1);
