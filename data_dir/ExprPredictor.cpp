...
for ( int k = 0; k < nUpdates; k++ ) {
 ...
 for ( int j = 0; j < seqs.size(); j++ ) {       
	...
	ofstream core2("corefile2NEW.txt");
		for( int jj = 0; jj < nSeqs ; jj++) {
			int start = pos[jj];
	...
	pwm_model2 = dcmm2.getInformation();  
		...
		if ( pwm_model2 > pwm_best ) {
			pwm_best = pwm_model2;	
			ofstream optcore("coreOPTNEW.txt");	
	...
	for(int shif = 0; shif < 0 ; shif++ ){
		...
		ostringstream numberm;
			numberm << shif;
				...
				ofstream core3(("corefile3NEW.txt" + numberm.str()).c_str());
		...
		dcFile =("corefile3NEW.txt" + numberm.str());
		Matrix dcm = countmatrixS( dcFile );
		Motif dcmm( dcm, background);
...
for ( int r = 0; r < nrandStarts; r++){
	...
	for(int shif = 0; shif < 0 ; shif++ ){
		ostringstream numberm;
		numberm << shif;
			ofstream core3(("coreOPTNEW.txt" + numberm.str()).c_str());
		for( int jj = 0; jj < nSeqs ; jj++) {
			int start = pos[jj];
			if(pos[jj] + l + shif > seqs[jj].size() ) { miss=miss + 1; continue; } 
	...
	dcFile =("coreOPTNEW.txt" + numberm.str());
    ...
    if ( format == FASTA ) {
        while ( getline( fin, line ) ) {
            ...
            if ( line[ 0 ] == '>' ) { 	
                ...
                for ( int i = start; i <= last+5; i++ ) {
			if( i > last ) { seq.push_back( symbolToInt( 'N' )); continue ; }
...
int l = enepwm.nRows();
    ...
    for ( int i = 0; i < enepwm.nRows(); i++ ) {
        for ( int j = 0; j < enepwm.nCols(); j++ ) {
            Expected_info = Expected_info - enepwm.getElement( i, j ) * log( enepwm.getElement(i,j) );	
        }	
    }
...
double Motif::getInformation( int nseqs)
{
Matrix enepwm = getPwm();
double Expected_info=0;
int l = enepwm.nRows();
    ...
    for ( int i = 0; i < enepwm.nRows(); i++ ) {
        for ( int j = 0; j < enepwm.nCols(); j++ ) {
		if( enepwm.getElement( i, j ) *(nseqs+4.0) >= nseqs +.1){
		Expected_info=Expected_info; continue;
		}
		if( enepwm.getElement( i, j ) *(nseqs+4.0) <= 1.1){
		Expected_info=Expected_info; continue;
		}
...
ostream& operator<<( ostream& os, const Site& site )
{
    ...
    os <<"\n"<< site.start + 1 << "\t" << strandChar <<  "\t" << site.information;
...
for(int i=0; i< seq.size(); i++ ){
	...
	size_t  found;
	 found=bst.find(ast);
 	 if (found!=string::npos){ p=p+1; tpou << ">" << seq_name[i] << endl<< seq[i] << endl<< ">" << seq_nameb[j] << endl<< seqb[j] << endl; continue;}
	
	if( j == seqb.size()-1 ){ n = n+1;   tnou << ">" << seq_name[i] << endl << seq[i] << endl << ">" << seq_nameb[j] << endl << seqb[j] << endl;  break;} 
...
void Sensitivity( const string& dc , const string& dc2 ){
...
ofstream ener( file2.c_str() );
		  for( int j = 0; j < seq.size() ; j++ ){
			...
			ener << seq[j] << " & " << fixed << setprecision(2) << energy <<"\\\\" << endl;  
		    }
...
Matrix countmatrixArich( const string& file )
{
...
Site t;
vector< double > ff(3);  
 vector< vector< vector< Site > > > sitespt(seq.size(), vector< vector< Site > >(2));
typedef  vector< Site > sitestt;	
...
for ( int e = 0; e < seq.size(); e++ ) {  
	...
	for ( int e = 0; e < seq.size(); e++ ) {  
	  for ( int k = 0; k < 1; k++ ) {  
		for( int i=0; i < sitespt[e][k].size() ; i++ ){
		if( sitespt[e][k].size() > 0 ) {
		sitespp[e][k].push_back( sitespt[e][k][i]  );  
...
for(int m = 0; m < ExprPredictor::seqsya.size(); m++ ) {
	...
	for( int i = 0; i < tsites.size() ; i++ ) {			
				...
				if( tsites[i].factorIdx == 0 ) { 
					...
					else{ dc << ALPHABET[complement(symbolToInt(ALPHABET[ ExprPredictor::seqsya[m][j+ motifs[ tsites[i].factorIdx ].length() -1 - p ] ]))];} 				
						} 
				...
				if( tsites[i].factorIdx == 3 ) { 
					 int j = 0;
					dcom << ">" << ExprPredictor::seqNmesa[m] << "_" << tsites[i].start <<endl ;
					for( int p = 0; p < motifs[ tsites[i].factorIdx ].length() ; p++ ){
						j = tsites[i].start;
					if(tsites[i].strand){
...
Matrix e = exprData;
	   ...
	   anny.annotydorsalZarend( seqsya, d, f, e, *func , thres, ExprPredictor::seqNmesa);
