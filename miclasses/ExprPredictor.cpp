...
double mutualinformatio9( const vector< Motif >& motifs, double alphadc )
{
vector< double > freqs;
...
double mi2=0;
int kk = 0;
vector< double > mixscores(0);
double maxmixscore = 0;
double tempmixscore=0;
double maxcbscore = 0;
double tempcbscore=0;
...
mi = mi + alphadc*pvalf8dc * log2(pvalf8dc/pvalf8) +(1-alphadc)*pvalf8du* log2(pvalf8du/pvalf8);
mi2= mi2 + alphadc*pvalf8dc * log2(pvalf8dc/(alphadc*pvalf8dc+ (1-alphadc)*pvalf8du)) +(1-alphadc)*pvalf8du* log2(pvalf8du/(alphadc*pvalf8dc+ (1-alphadc)*pvalf8du));
index = index +1;
mixscores.push_back( alphadc*pvalf8dc+ (1-alphadc)*pvalf8du );
tempmixscore=alphadc*pvalf8dc+ (1-alphadc)*pvalf8du ;
if(maxmixscore < tempmixscore ){
	maxmixscore = tempmixscore;
}
tempcbscore=pvalf8;
if(maxcbscore < tempcbscore ){
	maxcbscore = tempcbscore;
}
normcb = normcb + pvalf8;
normdc =normdc + pvalf8dc;
normdu =normdu + pvalf8du;
normcbm = normcbm + alphadc*pvalf8dc + (1-alphadc)*pvalf8du;
...
double mutualinformatio9cb( const vector< Motif >& motifs, double alphadc , string& file,string& file2 )
{
vector< double > freqs;
vector< double > freqscb;
vector< double > freqsdc;
...
ofstream seqtes("seqtest.txt");
		...
		seqtes<< "<" << j<<endl;
		seqtes<< seq[j] << endl;
					for ( int ii = 0; ii < seq[j].size(); ii++ ) {
			
			  
		 	    if ( ii + l > seq[j].size() ) break ;
		 	    double resultdc=1;
			    double resultdu=1;
			    Sequence elem( seq[j], ii, l, 1 );
		seqtes<< "<" << j<<endl;
		seqtes<< elem <<endl;
		
				double restemp =1;
		 	   for ( int i = 0; i < l; i++ ) {
					if( i < elem.size() ){if( elem[i] < 4 && elem[i] >= 0 ){
				       resultdc=resultdc*pwmdc(i,elem[ i ] );  
		
					resultdu=resultdu*pwmdu(i,elem[ i ] );
		
		
				    	}}
				   }
		
				double result = alphadc*resultdc + (1-alphadc)*resultdu;
				double energy = 0;
				energy = log2(maxmixscore/result);
				...
				seqtes<< "<" << j<<endl;
				seqtes<< energy <<endl;
		     		    Sequence rcElem( seq[j], ii, l, 0 );
		      	      	     
				       double resultdc2=1;
			               double resultdu2=1;
		
				    for ( int i = 0; i < l; i++ ) {
					if( i < rcElem.size() ){if( rcElem[ i ]  < 4 && rcElem[ i ]  >= 0 ){
				       
				    	resultdc2 *=pwmdc(i,elem[ i ] );  
					resultdu2*=pwmdu(i,elem[ i ] );
					}}
				   }
		
				double result2 = alphadc*resultdc2 + (1-alphadc)*resultdu2;
				double energy2 = 0;
				energy2 = log2(maxmixscore/result2);
				seqtes<< energy2 <<endl;
...
double mutualinformatio9cb2s( const vector< Motif >& motifs, double alphadc , string& file,string& file2 )
{
vector< double > freqs;
vector< double > freqscb;
vector< double > freqsdc;
...
int tempdcindex=0;
int tempduindex=0;
double tempduscore=0;
double tempdcscore=0;
double maxdcscore=0;
double maxduscore=0;
double tempmixscore=0;
double maxcbscore = 0;
double tempcbscore=0;
vector< int > nt(0);
ofstream fileofprob("fileofprob.txt");
ofstream fileofmixprob("fileofprobmix.txt");
ofstream fileofdcprob("fileofprobdc.txt");
ofstream fileofduprob("fileofprobdu.txt");
vector< double>mixpro(0);
vector< double>cbpro(0);
vector< double >dcpro(0);
vector<double>dupro(0);
vector<double > pr0(4,0);
vector<double > pr1(4,0);
vector<double > pr2(4,0);
vector<double > pr3(4,0);
vector<double > pr4(4,0);
vector<double > pr5(4,0);
vector<double > pr6(4,0);
vector<double > pr7(4,0);
vector<double > pr8(4,0);
double entmix =0;
double entcb =0;
double entdc =0;
double entdu =0;
...
mi = mi + alphadc*pvalf8dc * log2(pvalf8dc/pvalf8) +(1-alphadc)*pvalf8du* log2(pvalf8du/pvalf8);
mi2= mi2 + alphadc*pvalf8dc * log2(pvalf8dc/(alphadc*pvalf8dc+ (1-alphadc)*pvalf8du)) +(1-alphadc)*pvalf8du* log2(pvalf8du/(alphadc*pvalf8dc+ (1-alphadc)*pvalf8du));
tempcbindex = tempcbindex +1;
tempmixindex=tempmixindex+1;
fileofprob <<p0<<p1<<p2<<p3<< p4<<p5<<p6<<p7<<p8<<'\t'<< pvalf8 << endl;

tempmixscore=alphadc*pvalf8dc+ (1-alphadc)*pvalf8du ;
mixpro.push_back(tempmixscore);
cbpro.push_back(pvalf8);
fileofmixprob<<p0<<p1<<p2<<p3<< p4<<p5<<p6<<p7<<p8<<'\t'<< tempmixscore << endl;
if(maxmixscore < tempmixscore ){
	maxmixscore = tempmixscore;
	maxmixindex = tempmixindex;
}
tempcbscore=pvalf8;
if(maxcbscore < tempcbscore ){
	maxcbscore = tempcbscore;
	maxcbindex=tempcbindex;
}


tempdcindex = tempdcindex +1;
tempduindex=tempduindex+1;
tempdcscore=pvalf8dc;
tempduscore=pvalf8du;
dcpro.push_back(pvalf8dc);
dupro.push_back(pvalf8du);
fileofdcprob <<p0<<p1<<p2<<p3<< p4<<p5<<p6<<p7<<p8<<'\t'<< pvalf8dc << endl;

fileofduprob<<p0<<p1<<p2<<p3<< p4<<p5<<p6<<p7<<p8<<'\t'<< pvalf8du << endl;
if(maxdcscore < tempdcscore ){
	maxdcscore = tempdcscore;
	maxdcindex = tempdcindex;
}

if(maxduscore < tempduscore ){
	maxduscore = tempduscore;
	maxduindex=tempduindex;
}
normcb = normcb + pvalf8;
normdc =normdc + pvalf8dc;
normdu =normdu + pvalf8du;
normcbm = normcbm + alphadc*pvalf8dc + (1-alphadc)*pvalf8du;
entmix = entmix - tempmixscore*log2(tempmixscore);
entcb = entcb - tempcbscore*log2(tempcbscore);
entdc =entdc -tempdcscore*log2(tempdcscore);
entdu =entdu -tempduscore*log2(tempduscore);
pr0[p0]=pr0[p0] + tempmixscore;
pr1[p1]=pr1[p1] + tempmixscore;
pr2[p2]=pr2[p2] + tempmixscore;
pr3[p3]=pr3[p3] + tempmixscore;
pr4[p4]=pr4[p4] + tempmixscore;
pr5[p5]=pr5[p5] + tempmixscore;
pr6[p6]=pr6[p6] + tempmixscore;
pr7[p7]=pr7[p7] + tempmixscore;
pr8[p8]=pr8[p8] + tempmixscore;
...
fileofdcprob.close();
fileofduprob.close();
vector< vector< double > > ppp(9,vector< double >(4));
ppp[0]=pr0;
ppp[1]=pr1;
ppp[2]=pr2;
ppp[3]=pr3;
...
ostringstream numberm2;
//numberm2 << maxmixindex;
string gg = numberm2.str();

ostringstream numberm2b;
//numberm2b << maxcbindex;
string ggb = numberm2b.str();
...
ofstream seqtes("seqtest.txt");
		...
		seqtes<< elem <<endl;
				...
				ener << fixed << setprecision(2) << energy << endl;
				ener << fixed << setprecision(2) << energy2 << endl;
...
double mutualinformatio7( const vector< Motif >& motifs, double alphadc )
{
vector< double > freqs;
vector< double > freqscb;
vector< double > freqsdc;
...
mi = mi + alphadc*pvalf6dc * log2(pvalf6dc/pvalf6) +(1-alphadc)*pvalf6du* log2(pvalf6du/pvalf6);
mi2= mi2 + alphadc*pvalf6dc * log2(pvalf6dc/(alphadc*pvalf6dc+ (1-alphadc)*pvalf6du)) +(1-alphadc)*pvalf6du* log2(pvalf6du/(alphadc*pvalf6dc+ (1-alphadc)*pvalf6du));
index = index +1;

normcb = normcb + pvalf6;
normdc =normdc + pvalf6dc;
normdu =normdu + pvalf6du;
normcbm = normcbm + alphadc*pvalf6dc + (1-alphadc)*pvalf6du;
...
double mutualinformatio6( const vector< Motif >& motifs, double alphadc )
{
vector< double > freqs;
vector< double > freqscb;
vector< double > freqsdc;
...
pvalf0du = pwmdu(0,p0);
 ...
 f=f*(alphadc*pvalf5dc+ (1-alphadc)*pvalf5du);		
mi = mi + alphadc*pvalf5dc * log2(pvalf5dc/pvalf5) +(1-alphadc)*pvalf5du* log2(pvalf5du/pvalf5);
mi2= mi2 + alphadc*pvalf5dc * log2(pvalf5dc/(alphadc*pvalf5dc+ (1-alphadc)*pvalf5du)) +(1-alphadc)*pvalf5du* log2(pvalf5du/(alphadc*pvalf5dc+ (1-alphadc)*pvalf5du));
index = index +1;

normcb = normcb + pvalf5;
normdc =normdc + pvalf5dc;
normdu =normdu + pvalf5du;
normcbm = normcbm + alphadc*pvalf5dc + (1-alphadc)*pvalf5du;
}
}
}
}
...
double mutualinformatio5( const vector< Motif >& motifs, double alphadc )
{
vector< double > freqs;
vector< double > freqscb;
vector< double > freqsdc;
...
pvalf0du = pwmdu(0,p0);
	...
	double pvalf1du = pvalf0du*pwmdu(1,p1);
		...
		mi = mi + alphadc*pvalf4dc * log2(pvalf4dc/pvalf4) +(1-alphadc)*pvalf4du* log2(pvalf4du/pvalf4);
mi2= mi2 + alphadc*pvalf4dc * log2(pvalf4dc/(alphadc*pvalf4dc+ (1-alphadc)*pvalf4du)) +(1-alphadc)*pvalf4du* log2(pvalf4du/(alphadc*pvalf4dc+ (1-alphadc)*pvalf4du));
index = index +1;

normcb = normcb + pvalf4;
normdc =normdc + pvalf4dc;
normdu =normdu + pvalf4du;
normcbm = normcbm + alphadc*pvalf4dc + (1-alphadc)*pvalf4du;
}
}
}
}
...
double mutualinformatio4( const vector< Motif >& motifs, double alphadc )
{
vector< double > freqs;
vector< double > freqscb;
vector< double > freqsdc;
...
mi = mi + alphadc*pvalf3dc * log2(pvalf3dc/pvalf3) +(1-alphadc)*pvalf3du* log2(pvalf3du/pvalf3);
mi2= mi2 + alphadc*pvalf3dc * log2(pvalf3dc/(alphadc*pvalf3dc+ (1-alphadc)*pvalf3du)) +(1-alphadc)*pvalf3du* log2(pvalf3du/(alphadc*pvalf3dc+ (1-alphadc)*pvalf3du));
index = index +1;

normcb = normcb + pvalf3;
normdc =normdc + pvalf3dc;
normdu =normdu + pvalf3du;
normcbm = normcbm + alphadc*pvalf3dc + (1-alphadc)*pvalf3du;
}
}
}
}
...
Motif dcmm( countMatrix, pseudoCount, _background );
ofstream she2("beautydcdu.txt", ios::app);
she2 << dcmm ;
she2.close();
...
bool siteOverlapk( const Site& a, const Site& b, int k )
{
    if ( a.start + k <= b.start ) return false;
    if ( b.start + k <= a.start ) return false;
...
void scorematrixscanoverlap( Matrix e , string& file, double maxLLR, string& file2 )
{
    ifstream seq_file( file.c_str() );
    if ( !seq_file ) {
...
ofstream seqtes("seqtest.txt");
		...
		seqtes<< elem <<endl;
				...
				seqtes<< energy <<endl;
				    ...
				    for ( int i = 0; i < l; i++ ) {
					if( i < rcElem.size() ){if( rcElem[ i ]  < 4 && rcElem[ i ]  >= 0 ){
				       result2 += e( i, rcElem[ i ] ); 
				    	}}
				   }
				double energy2 = 0;
				energy2 = maxLLR - result2;
				seqtes<< energy2 <<endl;
...
void scorematrixscanoverlapboth( Matrix e , string& file, double maxLLR, string& file2 )
{
    ifstream seq_file( file.c_str() );
    if ( !seq_file ) {
...
void scorematrixscan( Matrix e , string& file, double maxLLR, string& file2 )
{
    ifstream seq_file( file.c_str() );
    if ( !seq_file ) {
...
vector< double > ff(3);  
 ...
 vector< vector< vector< Site > > > sitespp(seq.size(), vector< vector< Site > >(4));
...
for ( int ee = 0; ee < seq.size(); ee++ ) {  
		...
		seqtes<< "<" << ee<<endl;
		seqtes<< seq[ee] << endl;
     		for ( int ii = 0; ii < seq[ee].size(); ii++ ) {
			
			  
		 	    if ( ii + l > seq[ee].size() ) break ;
		 	    double result=0;
			    Sequence elem( seq[ee], ii, l, 1 );
				seqtes<< "<" << ee<<endl;
		seqtes<< elem <<endl;
		 	   for ( int i = 0; i < l; i++ ) {
					if( i < elem.size() ){if( elem[i] < 4 && elem[i] >= 0 ){
				...
				double energy = 0;
				energy = maxLLR - result;
			 	    sitespt[ee][0].push_back( Site( ii, 1, 0, energy ) );  
				...
				seqtes<< "<" << ee<<endl;
				seqtes<< energy2 <<endl;
				    sitespt[ee][0].push_back( Site( ii, 0, 0, energy2 ) ); 
				
				} 

			}
for ( int ee = 0; ee < seq.size(); ee++ ) {  
		while( !sitespt[ee][k].empty()){
			if( sitespt[ee][k].size() > 0 ) {
			sitespp[ee][k].push_back(  siteMaxscan(  sitespt[ee][k]  ));  
		
			sitesttoverlap( sitespt[ee][k], sitespp[ee][k] );  
			}
		}
		
	} 
for ( int ee = 0; ee < seq.size(); ee++ ) { 
	for(int i =0; i < sitespp[ee][k].size(); i++){
	
	ener << fixed << setprecision(2) << sitespp[ee][k][i].energy << endl;
	}
}
...
int ntemp =0;
	...
	ofstream cdfbackgrounddc("cdfbackgrounddca.txt");
	double pvalfb=0;
	for(int i = 0 ; i < bins2.size() ; i++){  
	...
	cdfbackgrounddc.close();
	ofstream pvalfile("backgroundCountsdca.txt");
...
ecounts.close();
	ofstream dusty("stranddu.txt");
	ofstream dcsty("stranddc.txt");
	ofstream cbstr("strandcb.txt");
	ofstream twstr("strandtw.txt");
for ( int e = 0; e < seq.size(); e++ ) {  
	  for ( int k = 0; k < motifs.size(); k++ ) {  
		...
		for ( int e = 0; e < seq.size(); e++ ) {  
			...
			if(k == 1 || k ==0 ){
			    ...
			    if( k==0 ){ co = co + 1; }
		 	    ...
		 	    double energy;
			    Sequence elem( seq[e], i, l, 1 );
		 	    energy = motifs[ k ].information(elem); 
				 if ( -energy <= energyThrs[ k ] ) {
				
				
			
			
			 	    sitespt[e][0].push_back( Site( i, 1, 0, -energy ) );  
				}
		     		    Sequence rcElem( seq[e], i, l, 0 );
		      	      	    energy = motifs[ k ].information(rcElem);       
				if ( -energy <= energyThrs[k] ) {	
				
				    sitespt[e][0].push_back( Site( i, 0, 0, -energy ) ); 
...
ofstream a("TPDC.txt",ios::app);
ofstream b("TPDU.txt",ios::app);
ofstream c("TPCB.txt",ios::app);
int ntpdc =0;
int ntpdu=0;
int ntpcb=0;
for ( int e = 0; e < seq.size(); e++ ) {  
		...
		ntpdc = sitespp[e][0].size() + ntpdc;
		ntpdu = sitespp[e][1].size() + ntpdu;
		ntpcb = sitespp[e][3].size() + ntpcb;
     		
	  
}
a << ntpdc << '\t' << energyThrs[ 0 ]<< endl;
b << ntpdu << '\t' << energyThrs[ 1 ]<< endl;
c << ntpcb << '\t' << energyThrs[ 3 ]<< endl;
a.close();
b.close();
c.close();
return 1;
...
int SeqAnnotator::annotydorsalZarend2uFP( vector< Sequence > seq, vector< vector< vector< Site > > >& sitespp , Matrix f, Matrix e, ExprFunc& p, int column, vector< string >  names, vector< vector< Site > >& seqSitesbot, vector< vector< Site > >& seqSitesm1)
{
Site t;
vector< double > ff(3);  
...
ofstream a("FPDC.txt",ios::app);
ofstream b("FPDU.txt",ios::app);
ofstream c("FPCB.txt",ios::app);
...
for ( int e = 0; e < seq.size(); e++ ) {  
		...
		nfpdc = sitespp[e][0].size() + nfpdc;
		nfpdu = sitespp[e][1].size() + nfpdu;
		nfpcb = sitespp[e][3].size() + nfpcb;
     		
	  
}
a << nfpdc << '\t' << energyThrs[ 0 ]<< endl;
b << nfpdu << '\t' << energyThrs[ 1 ]<< endl;
c << nfpcb << '\t' << energyThrs[ 3 ]<< endl;
a.close();
b.close();
c.close();
return 1;
...
void sitesttoverlap( vector< Site >& sitest, vector< Site >& sitesp )
{
for ( int i = 0; i < sitesp.size(); i++ ) {
	...
	while( ptr2 != sitest.end() ){
		if (siteOverlapk( sitesp[i], sitest[j], k) )  { 
...
Site siteMaxscan( vector< Site >& sitess)
{
 Site tempsite;
 double mine=100000;
...
vector< Sequence > ExprPredictor::seqsyaFP;
vector< string > ExprPredictor::seqNmesaFP;
...
if( b.start > a.start ) { dist =  abs( b.start - (a.start + motifs[0].length() ) );  }
double maxInt=1;
if( dist==0 ){ count = true;}
else{ 
if (      dist <B[0] )                 { maxInt = par.theV[a.factorIdx][b.factorIdx][ 0 ]; ExprFunc::Bct[0]++; } 
	if (      dist >=B[0] && dist <= B[1] ) { maxInt = par.theV[a.factorIdx][b.factorIdx][ 1 ]; ExprFunc::Bct[1]++; }
	if (      dist > B[1] && dist <= B[2] ) { maxInt = par.theV[a.factorIdx][b.factorIdx][ 2 ]; ExprFunc::Bct[2]++; 
	...
	if (      dist > B[2] && dist <= B[3] ) { maxInt = par.theV[a.factorIdx][b.factorIdx][ 3 ]; ExprFunc::Bct[3]++; }
	if (      dist > B[3] )                 { maxInt = par.theV[a.factorIdx][b.factorIdx][ 4 ]; ExprFunc::Bct[4]++; }
} 
...
if( dist==0 ){ count = true; }
	...
	dct << dist << '\t' ;
	if (      dist <B[0] )                 { maxInt = par.theV[a.factorIdx][b.factorIdx][ 0 ]; ExprFunc::B12[0]++; } 
		if (      dist >=B[0] && dist <= B[1] ) { maxInt = par.theV[a.factorIdx][b.factorIdx][ 1 ]; ExprFunc::B12[1]++; }
		if (      dist > B[1] && dist <= B[2] ) { maxInt = par.theV[a.factorIdx][b.factorIdx][ 2 ]; ExprFunc::B12[2]++; }
		if (      dist > B[2] && dist <= B[3] ) { maxInt = par.theV[a.factorIdx][b.factorIdx][ 3 ]; ExprFunc::B12[3]++; }
		if (      dist > B[3] )                 { maxInt = par.theV[a.factorIdx][b.factorIdx][ 4 ]; ExprFunc::B12[4]++; }
...
ofstream du( "DorsalUnCondit2.txt" );
ofstream dc( "DorsalCondit2.txt" );
for ( int e = 0; e < _d.size(); e++ ) {  
...
{
ofstream tw( "TwistCondit24.txt");
ofstream du( "DorsalUnCondit24.txt" );
ofstream dc( "DorsalCondit24.txt" );
ofstream dcom( "DorsalCombin24.txt" );
ofstream du3( "DUCondit3.txt" );
ofstream dc3( "DCondit3.txt" );
vector< Site > tsites;
for(int m = 0; m < ExprPredictor::seqsya.size(); m++ ) {
	...
	for( int i = 0; i < tsites.size() ; i++ ) {			
				...
				if(  tsites[i].start+motifs[ tsites[i].factorIdx ].length() + 2 > ExprPredictor::seqsya[m].size() ){continue;}
					 int j = 0;
					dc3 << tsites[i].energy << endl;
					dc << ">" << ExprPredictor::seqNmesa[m] << "_" << tsites[i].start << "_" << tsites[i].strand<<endl ;
					int shift = motifs[ tsites[i].factorIdx ].length()+4;
...
double ExprPredictor::compAvgCorrFP(  ExprPar& par, int thres ) 
{
...
Matrix e = exprData;
	   ...
	   anny.annotydorsalZarend2uFP( seqsyaFP, d, f, e, *func , thres, ExprPredictor::seqNmesaFP, seqSitesbot, seqSitesm1);
