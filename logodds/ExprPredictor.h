...
public:	
    ...
    Motif( const Matrix& _pwm, Motif& _motif ); // _pwm is conditional pwm (normalized frequencies); _motif is CB motif.
// this constructor builds the conditional motif (i.e. shifted energies of CB)
...
double getRMSEdc( string& file ) ;
  ...
  double PvalueSumf( double alpha ) ;  // constructs score cdf using selection fequencies (as opposed to backgroud)
    ...
    double PvalueSum( double alpha );   // length 11 site
	    double PvalueSumCond( double alpha );   // length 11 site
...
Matrix countmatrix2w( string& file, string& file2 , Motif _motif );// file contains sequences to construct llr, file2 is filled with llr scores (energy) of the first file sequence
