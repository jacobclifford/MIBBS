		...
		for ( int e = 0; e < seq.size(); e++ ) {  
			...
			if(k == 1 || k ==0 ){
			    ...
			    if( k==0 ){ co = co + 1; }
		 	    ...
		 	    double energy;
			    Sequence elem( seq[e], i, l, 1 );
		 	    energy = motifs[ k ].energy( elem ) + motifs[3].getMaxLLR()-motifs[k].getMaxLLR() ;
				 if ( energy <= energyThrs[ k ] ) {
				
				
			 	    sitespt[e][0].push_back( Site( i, 1, 1, energy ) );  
				}
		     		    Sequence rcElem( seq[e], i, l, 0 );
		      	      	    energy = motifs[ k ].energy( rcElem )+ motifs[3].getMaxLLR()-motifs[k].getMaxLLR();
				if ( energy <= energyThrs[k] ) {	
				
				    sitespt[e][0].push_back( Site( i, 0, 1, energy ) ); 
				}
